...
- **自主模式**：team lead 自行确认，在最终发布确认时一并展示
...
|---------|---------|---------|
| **Low** | >= 8 | 正常发布 |
| **Medium** | 6 ~ 8 | 标准模式：提示风险后确认；自主模式：继续并高亮风险 |
...
用 AskUserQuestion 等待用户最终确认。此确认**不可跳过**，即使是自主模式也必须确认。
...
| 发布类型推断不确定 | 标准模式：AskUserQuestion 让用户选择；自主模式：选择较高的类型（偏保守） |
